...
/**
 * Намира първия свободен интервал в текущия календар.
 */
public class FindSlotCommand implements Command {
     ...
     */
    public FindSlotCommand() {
    }

    /**
     * Изпълнява командата findslot.
     *
    ...
    public void execute(String[] args, CalendarContext context, CalendarService service, Scanner consoleScanner) throws Exception {
        if (args.length != 3) {
            throw new InvalidCommandException("Употреба: findslot <yyyy-MM-dd> <durationHours>");
        }
        LocalDate fromDate = LocalDate.parse(args[1]);
        int durationHours = Integer.parseInt(args[2]);
        TimeSlot slot = service.findSlot(context, fromDate, durationHours);
        if (slot == null) {
            System.out.println("Не е намерен свободен интервал.");
        } else {
            System.out.println("Първи свободен интервал: " + slot);
        }
    }
}
